...
static func is_any_point_on_screen(node: Node2D, shape_obj: Variant) -> bool:
	var screen_size := ScreenGlobal.screen_size
	var points: PackedVector2Array = _dispatch_to_points(shape_obj)
	for p in points:
...
static func _dispatch_to_points(shape_obj: Variant) -> PackedVector2Array:
	var res: PackedVector2Array
	if shape_obj is Polygon2D:
		res = shape_obj.polygon
	if shape_obj is CollisionShape2D:
